...
function getGradientColors(baseColor: string | null, defaultPrimary: string): string[] {
  let color = baseColor || defaultPrimary;
  
  // If it's a short 3-character hex (e.g. #fff), expand it to 6-character hex
  if (color.startsWith('#') && color.length === 4) {
    color = '#' + color[1] + color[1] + color[2] + color[2] + color[3] + color[3];
  }
  
  // Safe validation check: fall back to default primary if not a valid 6-char hex
  ...
  const toHex = (c: number) => (isNaN(c) ? 0 : c).toString(16).padStart(2, '0');
    ...
    <View style={styles.verticalAccountsList}>
      {accountsList.map((acc) => (
        <Pressable
          key={acc.id}
          onPress={() => onAccountPress?.(acc)}
          style={styles.verticalAccountWrapper}
          accessibilityLabel={`Edit account ${acc.name}`}
          accessibilityRole="button"
        >
          <GradientCard
